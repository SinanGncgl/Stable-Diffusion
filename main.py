import argparse, os, sys, datetime, glob  # noqa
import numpy as np
import time
import torch
import torchvision
...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _val_dataloader(self, shuffle=False):
        if False or self.use_worker_init_fn:
            init_fn = worker_init_fn
        else:
            init_fn = None
        return DataLoader(
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not "gpus" in trainer_config:  # noqa
        ...
        if not opt.finetune_from == "":
            ...
            if in_shape != in_filters_load.shape:
                ...
                for k in keys_to_change:
                    ...
                    old_state[k] = modify_weights(
                        old_state[k], scale=scale, n=in_shape // 4 - 1
                    )
        ...
        if not "plugins" in trainer_kwargs:  # noqa
        ...
        if opt.scale_lr:
            ...
            rank_zero_print(
                "Setting learning rate to {:.2e} = {} (accumulate_grad_batches) * {} (num_gpus) * {} (batchsize) * {:.2e} (base_lr)".format(  # noqa
                    model.learning_rate, accumulate_grad_batches, ngpu, bs, base_lr
                )
            )
        else:
