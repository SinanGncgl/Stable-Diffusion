...
class DDPM(pl.LightningModule):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool):
        ...
        if self.parameterization == "eps":
            x_recon = self.predict_start_from_noise(x, t=t, noise=model_out)  # noqa
        elif self.parameterization == "x0":
            x_recon = model_out
        if clip_denoised:
    ...
    def log_images(
        ...
        **kwargs,
        ...
        if inpaint:
            ...
            mask[:, h // 4 : 3 * h // 4, w // 4 : 3 * w // 4] = 0.0  # noqa
            mask = mask[:, None, ...]
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        if self.conditioning_key is None:
            ...
            cc = torch.cat(c_crossattn, 1)  # [B, 1, 768]
            # x shape: [B, 4, 10, 10]
...
class LatentInpaintDiffusion(LatentDiffusion):
    ...
    def __init__(
        ...
        keep_finetune_dims=4,  # if model was trained without concat mode before and we would like to keep these ch
            ...
            if exists(self.finetune_keys) and k in self.finetune_keys:
                ...
                for name, param in self.named_parameters():
                    if name in self.finetune_keys:
                        print(
                            f"modifying key '{name}' and keeping its original {self.keep_dims} (ch) dimensions only"
                        )
...
class Layout2ImgDiffusion(LatentDiffusion):
    ...
    def log_images(self, batch, N=8, *args, **kwargs):
        ...
        bbox_imgs = []
        map_fn = lambda catno: dset.get_textual_label(  # noqa
...
class MultiCatFrameDiffusion(LatentDiffusion):
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, log_mode=False):
        ...
        cats = []
        for i in range(n):
            x_low = cat_conds[:, :, :, 3 * i : 3 * (i + 1)]  # noqa
            x_low = rearrange(x_low, "b h w c -> b c h w")
            x_low = x_low.to(memory_format=torch.contiguous_format).float()
            encoder_posterior = self.encode_first_stage(x_low)
            zx = self.get_first_stage_encoding(encoder_posterior).detach()
