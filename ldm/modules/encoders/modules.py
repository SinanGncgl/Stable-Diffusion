...
from ldm.models.resnet_embedder import resnet12
    ...
    def forward(self, img):
        ...
        with torch.no_grad():
            ...
            if self.retreival_key:
                ...
                face = img[:, 3:, 190:440, x_offset : (512 - x_offset)]  # noqa
                other = img[:, :3, ...].clone()
            else:
                face = img[:, :, 190:440, x_offset : (512 - x_offset)]  # noqa
                other = img.clone()

            if self.augment:
                face = K.RandomHorizontalFlip()(face)

            other[:, :, 190:440, x_offset : (512 - x_offset)] *= 0  # noqa
            encodings = [
...
class FrozenResNetImageEmbedder(AbstractEncoder):
    def __init__(
        self,
        device="cpu",
